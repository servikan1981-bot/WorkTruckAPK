...
public class TourActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        tb.addView(sceneCounter);tb.addView(sceneTitle);tb.addView(sceneSub);
        ...
        TextView dot=tv("●",18,Color.WHITE,true);dot.setGravity(Gravity.CENTER);
        ...
        np.setMargins(0,0,0,dp(10));root.addView(nav,np);

        TextView help=tv("Тяните пальцем для обзора · двумя пальцами — масштаб",10,Color.WHITE,false);
        help.setGravity(Gravity.CENTER);help.setPadding(dp(12),dp(6),dp(12),dp(6));
        ...
        hp.setMargins(0,0,0,dp(73));root.addView(help,hp);

        setContentView(root);go(0);
    }
    ...
    private void go(int n){
        ...
        scene=n;panorama.setScene(n);
        sceneCounter.setText("ТОЧКА "+(n+1)+" ИЗ "+titles.length);
        sceneTitle.setText(titles[n]);sceneSub.setText(subs[n]);
        prev.setEnabled(n>0);next.setEnabled(n<titles.length-1);
        prev.setAlpha(n>0?1f:.35f);next.setAlpha(n<titles.length-1?1f:.35f);
    }
    ...
    private class PanoramaView extends View{
        ...
        PanoramaView(){
            ...
            scale=new ScaleGestureDetector(TourActivity.this,new ScaleGestureDetector.SimpleOnScaleGestureListener(){
                @Override public boolean onScale(ScaleGestureDetector d){zoom=Math.max(1f,Math.min(3.2f,zoom*d.getScaleFactor()));clampPan();invalidate();return true;}
            });
        }

        void setScene(int s){
            if(world!=null&&!world.isRecycled())world.recycle();
            BitmapFactory.Options o=new BitmapFactory.Options();o.inPreferredConfig=Bitmap.Config.ARGB_8888;o.inScaled=false;o.inDither=true;
            world=BitmapFactory.decodeResource(getResources(),images[s],o);pan=0.5f;zoom=1f;invalidate();
        }

        @Override protected void onDraw(Canvas c){
            super.onDraw(c);if(world==null)return;
            int vw=getWidth(),vh=getHeight();if(vw<=0||vh<=0)return;
            float viewRatio=(float)vw/vh;
            float cropH=world.getHeight()/zoom;
            float cropW=cropH*viewRatio;
            if(cropW>world.getWidth()){cropW=world.getWidth();cropH=cropW/viewRatio;}
            cropW=Math.max(1,Math.min(cropW,world.getWidth()));cropH=Math.max(1,Math.min(cropH,world.getHeight()));
            float maxLeft=Math.max(0,world.getWidth()-cropW);
            float left=maxLeft*pan;float top=Math.max(0,(world.getHeight()-cropH)/2f);
        ...
        @Override public boolean onTouchEvent(MotionEvent e){
            ...
            if(e.getPointerCount()==1){
                ...
                if(e.getAction()==MotionEvent.ACTION_MOVE){float dx=e.getX()-lastX;pan-=dx/Math.max(1,getWidth())*.72f/zoom;clampPan();lastX=e.getX();invalidate();return true;}
            }
            return true;
        }
    }
    ...
    private Button button(String x){Button b=new Button(this);b.setText(x);b.setTextColor(Color.WHITE);b.setTextSize(12);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setBackground(round(GREEN,14));return b;}
