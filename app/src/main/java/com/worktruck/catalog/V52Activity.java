...
public class V52Activity extends Activity {
    ...
    private View categoryGrid(boolean counts){
        ...
        if(counts&&dbReady) map=database.getCategoryCounts();
        for(String cat:Database.CATEGORIES){
            String label=cat;
            if(counts){int n=map.containsKey(cat)?map.get(cat):0;label=cat+"\n"+nf.format(n)+" поз.";}
            g.addView(categoryCard(categoryIcon(cat),label,()->showProducts(cat,"")),gridCatLp());
        }
        return g;
    }
    ...
    private void showCategories(){
        ...
        TextView sub=tv("Все товары уже заранее распределены по названию. Нажмите раздел — позиции появятся сразу.",14,MUTED,false); sub.setPadding(0,0,0,dp(12)); b.addView(sub);
        b.addView(categoryGrid(true));
        sv.addView(b); root.addView(sv,new LinearLayout.LayoutParams(-1,0,1)); root.addView(bottomNav());
    }
    ...
    private void showProducts(String category,String initial){
        ...
        loadPage[0]=()->{
            int token=serial[0]; String q=search.getText().toString().trim(); String w=String.valueOf(wh.getSelectedItem()); int offset=loaded[0];
            if(!dbReady){ count.setText("Каталог недоступен"); return; }
            new Thread(()->{
                try{
                    List<Product> data=database.searchCategoryPage(category,q,w,pageSize,offset);
                    runOnUiThread(()->{
                        if(token!=serial[0]||isFinishing())return;
                        removeLoadMore(results);
                        for(Product p:data) results.addView(productCard(p),lp(-1,-2,0,0,0,8));
                        loaded[0]+=data.size();
                        count.setText("Показано "+nf.format(loaded[0])+" из "+nf.format(total[0])+" позиций");
                        if(loaded[0]<total[0]){
                            Button more=greenButton("ПОКАЗАТЬ ЕЩЁ 100"); more.setTag("load_more"); more.setOnClickListener(v->loadPage[0].run()); results.addView(more,lp(-1,dp(52),0,6,0,12));
                        }
                        if(total[0]==0){ TextView e=tv("Ничего не найдено.",15,MUTED,false);e.setGravity(Gravity.CENTER);e.setPadding(dp(12),dp(30),dp(12),dp(30));results.addView(e); }
                    });
                }catch(Throwable ex){ runOnUiThread(()->count.setText("Ошибка загрузки каталога")); }
            }).start();
        };

        Runnable restart=()->{
            int token=++serial[0]; loaded[0]=0; results.removeAllViews(); count.setText("Загрузка…"); String q=search.getText().toString().trim(); String w=String.valueOf(wh.getSelectedItem());
            if(!dbReady){count.setText("Каталог недоступен");return;}
            new Thread(()->{
                try{ int t=database.countMatches(category,q,w); runOnUiThread(()->{if(token!=serial[0])return;total[0]=t;loadPage[0].run();}); }
                catch(Throwable ex){runOnUiThread(()->count.setText("Ошибка поиска"));}
            }).start();
        };

        search.setOnEditorActionListener((v,id,e)->{restart.run();return true;});
        wh.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onNothingSelected(AdapterView<?> p){}public void onItemSelected(AdapterView<?> p,View v,int pos,long id){restart.run();}});
        restart.run();
    }

    private void removeLoadMore(LinearLayout results){
        for(int i=results.getChildCount()-1;i>=0;i--){View v=results.getChildAt(i);if("load_more".equals(v.getTag()))results.removeViewAt(i);}
    }
