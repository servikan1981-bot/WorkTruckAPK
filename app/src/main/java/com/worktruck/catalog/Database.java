...
public class Database {
    ...
    public synchronized Map<String,Integer> getCategoryCounts(){
        ...
        for(String c:CATEGORIES) out.put(c,0);
    ...
    public synchronized int countMatches(String category,String query,String warehouse){
        if(db==null||!db.isOpen()) return 0;
        QueryParts qp=buildWhere(category,query,warehouse);
        try(Cursor c=db.rawQuery("SELECT count(*) FROM products WHERE "+qp.where,qp.args.toArray(new String[0]))){
    ...
    public synchronized List<Product> searchCategoryPage(String category,String query,String warehouse,int limit,int offset){
        ...
        try(Cursor c=db.rawQuery(sql,qp.args.toArray(new String[0]))){
            while(c.moveToNext()){
                ...
                p.id=c.getLong(0); p.article=s(c.getString(1)); p.manufacturerNo=s(c.getString(2));
                p.manufacturer=s(c.getString(3)); p.brand=p.manufacturer; p.name=s(c.getString(4));
                p.price=c.isNull(5)?0:c.getDouble(5); p.qty=c.isNull(6)?0:c.getDouble(6);
                p.warehouse=s(c.getString(7)); p.series=s(c.getString(8)); out.add(p);
            }
        }
        return out;
    }
    ...
    private static class QueryParts{
        final String where; final ArrayList<String> args;
        QueryParts(String w,ArrayList<String> a){where=w;args=a;}
    }
