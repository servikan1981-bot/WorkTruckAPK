...
PY
adb install -r /tmp/family-618.apk
...
sleep 7
adb logcat -d -b crash > /tmp/family-618-crash.log
if grep -A 4 'FATAL EXCEPTION' /tmp/family-618-crash.log | grep -q "Process: $pkg"; then cat /tmp/family-618-crash.log; exit 1; fi
test -n "$(adb shell pidof "$pkg")"
adb exec-out screencap -p > /tmp/family-618-upgrade.png
echo "PASS: Android ${FAMILY_API_LEVEL} upgrade 6.0.17 -> 6.0.18 launches"
