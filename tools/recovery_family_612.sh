...
for label,candidate in [('old','611-test.json'),('new','612.json')]:
...
test "$(adb shell dumpsys package "$pkg" | sed -n 's/.*versionCode=\([0-9]*\).*/\1/p' | head -1 | tr -d '\r')" = 6012
adb shell am force-stop "$pkg"
adb logcat -c
adb shell am start -W -n "$pkg/$act"
sleep 8
adb logcat -d -b crash > /tmp/family-612-crash.log
if adb logcat -d -b crash | grep -A 4 'FATAL EXCEPTION' | grep -q "Process: $pkg"; then cat /tmp/family-612-crash.log; exit 1; fi
adb shell pidof "$pkg"
python3 - <<'PY'
import sys
sys.path.insert(0,'tools')
...
PY
adb shell uiautomator dump /sdcard/family-612.xml >/dev/null
adb exec-out cat /sdcard/family-612.xml > /tmp/family-612.xml
adb exec-out screencap -p > /tmp/family-612.png
adb logcat -d -t 1500 > /tmp/family-612-logcat.txt
if ! grep -q 'Света' /tmp/family-612.xml; then
  sleep 12
  adb shell uiautomator dump /sdcard/family-612.xml >/dev/null
  adb exec-out cat /sdcard/family-612.xml > /tmp/family-612.xml
fi
grep -q 'Света' /tmp/family-612.xml
adb shell am force-stop "$pkg"
adb shell am start -W -n "$pkg/$act"
sleep 5
test -n "$(adb shell pidof "$pkg")"
if adb logcat -d -b crash | grep -A 4 'FATAL EXCEPTION' | grep -q "Process: $pkg"; then adb logcat -d -b crash > /tmp/family-612-crash.log; cat /tmp/family-612-crash.log; exit 1; fi
