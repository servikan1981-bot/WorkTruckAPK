...
for label,candidate in [('old','612.json'),('new','613.json')]:
...
test "$(adb shell dumpsys package "$pkg" | sed -n 's/.*versionCode=\([0-9]*\).*/\1/p' | head -1 | tr -d '\r')" = 6013
adb shell am force-stop "$pkg"
adb logcat -c
adb shell am start -W -n "$pkg/$act"
sleep 8
adb logcat -d -b crash > /tmp/family-613-crash.log
if adb logcat -d -b crash | grep -A 4 'FATAL EXCEPTION' | grep -q "Process: $pkg"; then cat /tmp/family-613-crash.log; exit 1; fi
adb shell pidof "$pkg"
python3 - <<'PY'
import sys
sys.path.insert(0,'tools')
...
adb logcat -d -t 1500 > /tmp/family-613-logcat.txt
...
if adb logcat -d -b crash | grep -A 4 'FATAL EXCEPTION' | grep -q "Process: $pkg"; then adb logcat -d -b crash > /tmp/family-613-crash.log; cat /tmp/family-613-crash.log; exit 1; fi
